...
module.exports.split = function split(str, _, _bp) {
  ...
  */
  var parts = [], // holds the resulting parts
  ...
  while ((match = re.exec(str))) {
    ...
    if (isexpr) {
      ...
      */
      if (match[2]) {
        // if have a javascript opening bracket,
        re.lastIndex = skipBraces(str, match[2], re.lastIndex)
        continue // skip the bracketed block and loop
      }

      if (!match[3]) {
        // if don't have a closing bracket
    ...
    */
    if (!match[1]) {
      // ignore it if have an escape char
  ...
  if (str && start < str.length) {
    // push remaining part, if we have one
