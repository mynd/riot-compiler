...
function compile(src, opts, url) {
  ...
  if (!opts) opts = {}
  opts = { parser: (source) => source, ...opts }
  ...
  output = cleanSource(output).replace(CUST_TAG, function (_, indent, tagName, attribs, body, body2) {
    ...
    attribs =
      attribs && included('attribs') ? restoreExpr(parseAttribs(splitHtml(attribs, opts, pcex), pcex), pcex) : ''

    if ((body || (body = body2)) && /\S/.test(body)) {
      if (body2) throw new Error('assertion failed: body2 == null')
      ...
      if (blocks[1].trim() !== '') throw new Error("assertion failed: blocks[1] === ''")
    }
  ...
  if (opts.entities) {
    ...
    const $ = cheerio.load(parts[0].html, undefined, false)
    // console.log(
    //   $.root()
    //     .contents()
    //     .filter((_, e) => e.nodeType === 3 && e.data.trim() !== '')
    // )
    $('virtual').each((_, e) => {
      e.tagName = 'template555'
    })
    $('*').each((_, e) => {
      if ('if' in e.attribs) {
        delete e.attribs['if']
    ...
    if (parts.length !== 1) throw new Error('assertion failed: parts.length === 1')
    parts[0].vue = $.root().html()
    return parts[0]
  }
  ...
  if (opts.sourcemap) {
    throw new Error('assertion failed: opts.sourcemap should be falsy.')
  }
  return output
}
