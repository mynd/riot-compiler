 ...
 */
var HTML_TAGS =
  /(?:<(-?[A-Za-z][-\w\xA0-\xFF]*)(?:\s+([^"'/>]*(?:(?:"[^"]*"|'[^']*'|\/[^>])[^'"/>]*)*)|\s*)(\/?)>)|(?:<\/(-?[A-Za-z][-\w\xA0-\xFF]*)\s*>)/g
...
var RE_HASEXPR = safeRegex(/@#\d/, 'x01'),
  ...
  RE_REPEXPR_CAP = safeRegex(/@#(\d+)/, 'x01'),
  CH_IDEXPR = '\x01#',
  CH_DQCODE = '\u2057',
  DQ = '"',
  SQ = "'"
...
function splitHtml(html, opts, pcex) {
  ...
  if (html && _bp[4].test(html)) {
    var list = brackets.split(html, 0, _bp),
      expr
    ...
    for (var i = 1; i < list.length; i += 2) {
      ...
      if (expr[0] === '^') {
        ...
        expr = expr.trim()
        expr = expr
          .replace(/\bSTRINGRSC\b/g, '$$t')
          .replace(/\breadable\b/g, 'hasPermissionToRead')
          .replace(/\beditable\b/g, 'hasPermissionToEdit')
          .replace(/\bediting\b/g, 'isEditing')
          .replace(/\baddable\b/g, 'isAddable')
          .replace(/\bitemNum\b/g, 'listGrandTotal')
          .replace(/\blimit\b/g, 'listLimit')
        expr = `{ ${expr} }`
      }
...
const tagNameTable = {
  ...
  importbutton: {
    ngName: 'import-button',
  },
  exportbutton: {
    ngName: 'export-button',
  },
  grid: {
    ngName: 'plex-grid',
  },
  ...
  detailheader: {
    ngName: 'detail-header',
  },
}
const FEATURETITLE_CLICK_EXP = /onTransition\.bind\(this,\s*(\S+)\)/
const DOLLAR_T_FUNC_ONE_QUOTED_ARG = /\$t\('([^),']+)'\)/
const SQ_STRING = /'([^']*)'/
function _compileHTML(html, opts, pcex) {
  ...
  const enclosedBy = (name) => tagStack.find((x) => x.name === name)

  html = splitHtml(html, opts, pcex).replace(HTML_TAGS, function (_, name, attr, ends, closeName, offset, entire) {
    closeName, offset, entire
    if (_.startsWith('</')) {
      let name =
        closeName in tagNameTable && 'ngName' in tagNameTable[closeName] ? tagNameTable[closeName]['ngName'] : closeName
      tagStack.pop()
      return `</${name}>`
    }
    name = name.toLowerCase()
    tagStack.push({ name, offset, length: _.length })

    ends = ends && !VOID_TAGS.test(name) ? '></' + name : ''

    let attribs = parseAttribs(attr, pcex)

    attribs.forEach((a) => {
      if (a.length > 1) {
        const value = a[1].slice(1, -1)
        const m = value.match(RE_REPEXPR_CAP)
        if (m != null) {
          if (a[0].startsWith('on')) {
            a[0] = `@${a[0].slice(2)}`
          } else if (a[0] === 'if') {
            a[0] = 'v-if'
          } else {
          ...
          if (a[0] === ':class') {
            a[1] = `${DQ}${pcex[m[1]].replace(/([\w-]+):/g, `${SQ}$1${SQ}:`)}${DQ}`
          } else {
            a[1] = `${DQ}${pcex[m[1]].slice(1, -1).trim()}${DQ}`
          }
        }
      }
    })
    ...
    attribs.forEach((a, i) => {
      if (a[0] === ':titletext' && name === 'featuretitle' && a[1] != null) {
        const m = a[1].slice(1, -1).match(DOLLAR_T_FUNC_ONE_QUOTED_ARG)
        if (m != null) {
          a[0] = 'titletext'
          a[1] = `${DQ}${m[1]}${DQ}`
        }
      } else if (a[0] === ':manual' && name === 'featuretitle' && a[1] != null) {
        const m = a[1].slice(1, -1).match(SQ_STRING)
        if (m != null) {
          a[0] = 'manual'
          a[1] = `${DQ}${m[1]}${DQ}`
        }
      } else if (a[0] === 'to' && name === 'yield' && a[1] != null) {
        attribs[i] = [`#${a[1].slice(1, -1)}`]
      } else if (a[0] === '@click' && name === 'a' && enclosedBy('featuretitle') && a[1] != null) {
        const m = a[1].slice(1, -1).match(FEATURETITLE_CLICK_EXP)
        if (m != null) {
          a[1] = `${DQ}selectCrumb(${m[1]})${DQ}`
        }
      } else if (a[0] === 'data-rt-floatingarea') {
        a[0] = 'ref'
        a[1] = '"floatingarea"'
      }
    })

    if (name === 'importbutton') {
      attribs = [
        [':gridoptions', '"gridoptions"'],
        [':title', '"$t(\'TODO:\')"'],
        ['limit-key', '"TODO:"'],
        ['preview-cancel-signature', '"TODO:"'],
        ['preview-commit-signature', '"TODO:"'],
        ['preview-signature', '"TODO:"'],
        ['upload-signature', '"TODO:"'],
      ]
    } else if (name === 'exportbutton') {
      attribs = [
        ['download-signature', '"TODO:"'],
        [':title', '"$t(\'TODO:\')"'],
      ]
    } else if (name === 'grid') {
      attribs = [
        [':gridoptions', '"gridoptions"'],
        [':items', '"list"'],
        [':selected', '"TODO: may be removed"'],
        ['@select', '"TODO: may be removed"'],
      ]
    }

    let nameAndAttrs = name in tagNameTable && 'ngName' in tagNameTable[name] ? tagNameTable[name]['ngName'] : name
    if (attribs.length > 0)
      nameAndAttrs += ' ' + attribs.map((a) => (a.length === 1 ? a[0] : `${a[0]}=${a[1]}`)).join(' ')

    if (VOID_TAGS.test(name)) {
      tagStack.pop()
    }

    return '<' + nameAndAttrs + ends + '>'
  })
...
function _compileCSS(css, tag, type, opts) {
  ...
  if (tag && (!opts.parserOpts || opts.parserOpts.prefixCSS)) css = scopedCSS(tag, css)
...
function mktag(name, html, css, attr, js, imports, opts) {
  ...
  return (
    imports + "riot.tag2('" + name + SQ + c + _q(html, 1) + c + _q(css) + c + _q(attr) + ', function(opts) {\n' + js + s
  )
}
...
function compile(src, opts, url) {
  ...
  output = CUST_TAG.exec(output)[0]
  output = cleanSource(output).replace(CUST_TAG, function (_, indent, tagName, attribs, body, body2) {
    var jscode = '',
      styles = '',
      html = '',
      imports = '',
    ...
    if ((body || (body = body2)) && /\S/.test(body)) {
      if (body2) {
        ...
        body = body.replace(SCRIPTS, function (_m, _attrs, _script) {
          if (included('js')) {
            var code = getCode(_script, opts, '', url)
